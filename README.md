# NISAR Data Management System (NDMS) — Summary

NDMS is a compact analytics platform for satellite observation data. It ingests MySQL/MariaDB sources, stages raw data in a PostgreSQL `raw` schema, runs a two-phase ETL (raw load → transform), and publishes read-optimized analytics in `public` for a web dashboard and AI-assisted querying.

Key facts:
- Developed: 2026
- Source data range: 2025-09-23 to present
- Core components: Flask backend (API + ETL service), ETL pipeline, PostgreSQL analytics, lightweight JS frontend (map, charts, tables)
- Containers: docker-compose orchestration (backend, etl, dbs, frontend)

Quick start:
1. Review service configs in `docker-compose.yml`.
2. Backend: [backend/app.py](backend/app.py) — Flask API server.
3. ETL: [etl/etl_service.py](etl/etl_service.py) — Phase 1/2 trigger endpoints.
4. Frontend: `frontend/src/index.html` and `frontend/src/js/` for UI components.

If you need a more detailed doc or specific endpoints for Clouade AI, tell me which parts to expand (API list, schema, or ETL steps).
    }
  ],
  "page": 1,
  "total_pages": 42,
